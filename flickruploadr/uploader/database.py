...
class FlickrDatabase():
    ...
    def __init__(self, obj):
        self.flickrcore = obj
        self.logger = common.create_logger('FlickrDB')

        # Start connection to database
        self.logger.info('Starting FlickrDB')
        self.progress(msg2='Starting FlickrDB')
        ...
        self.progress(msg2='Creating database tables')
    ...
    async def rebuild_database(self, local=False):
        ...
        self.progress(msg2='Finished updating database')
        return True
    ...
    def check_machine_tags(self, photo):
        ...
        try:
            ...
            if self.check_hash(md5, 'md5') is False or self.check_hash(sha1, 'sha1') is False:
                ...
                # print(photo_id)
                self.progress(msg1='Malformed tags', msg2='Downloading photo from flickr')
            ...
            if md5 == '' or sha1 == '':
                ...
                # print(photo_id)
                self.progress(msg1='Incomplete tags', msg2='Downloading photo from flickr')
                md5, sha1 = self.download_flickr_photo(photo_id)
        ...
        # No machine tags available
            ...
            # print(photo_id)
            self.progress(msg1='No tags found', msg2='Downloading photo from flickr')
            md5, sha1 = self.download_flickr_photo(photo_id)
    ...
    def download_flickr_photo(self, photo_id, temp=True, folder=''):
        ...
        if temp:
            ...
            self.progress(msg2='Downloading photo from Flickr')
    ...
    def write_flickr_photo(self, obj_photo=None, dict_photo=None, table=''):
        if obj_photo is not None:
            ...
            if photo_title == '' and description is not None:
                ...
                self.progress(msg2='Setting "{}" description ("{}") as title'.format(photo_id, description))
            ...
            self.progress(md5=md5, sha1=sha1)
    ...
    def write_flickr_album(self, obj_album=None, dict_album=None, update=True, verbose=True):
        ...
        self.progress(album=album_title, album_id=album_id)
        # Write album into database
        if verbose:
            self.logger.debug('Add album to albums table')
            pass
        ...
        if verbose:
            ...
            self.progress(msg2='Retrieve photos for "{}"'.format(album_title))
            pass

        if update is True:
            self.logger.debug('Updating photos in album "{}"'.format(album_title))
            self.progress(msg2='Updating photos in album "{}"'.format(album_title))
    ...
    async def list_albums(self, per_page=500, update=True, verbose=True):
        ...
        for set_page in range(total_album_pages):
            ...
            for set in set_elements:
                ...
                # Update GUI
                self.progress(msg2='Updating albums', actual_album=set_count)
    ...
    async def list_photos(self, per_page=500):
        ...
        for photo_page in range(total_photo_pages):
            ...
            for photo in photo_elements:
                ...
                # Update GUI
                self.progress(msg1='Updating photos', actual_image=photo_cnt)
