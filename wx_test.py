...
class MainFrame(wx.Frame):
    def __init__(self, parent):
        wx.Frame.__init__(self, parent,size=(650, 485))
        ...
        self.user = wx.ComboBox(self.panel, choices=list(users.keys()), name='userCombo', pos=(h2, vs+(0*vo)), size=(300,25))
        ...
        self.fname = wx.StaticText(self.panel,      label="",             pos=(h2,  vs+( 4*vo)), size=(200,25))
        ...
        self.listBox1 = wx.ListBox(choices=[],      name='listBox1',      pos=(h1, vs+(12*vo)),  size=(400, 125), style=0, parent=self.panel)

        self.btnUpload = wx.Button(self.panel,      label="Start upload", pos=( 5, vs+(17.25*vo)), size=(200, 20))
        self.btnUpdate = wx.Button(self.panel,      label="Start update", pos=(275, vs+(17.25*vo)), size=(125, 20))
    ...
    def callUpload(self, evt):
        ...
        print('\nWx GUI: Privacy settings:\n\tPublic={}\n\tFamily={}\n\tFriends={}\n'.format(self.cb['public'],self.cb['family'],self.cb['friends']))
        
        startUpload(username=username, root_dir=root_dir, subdir=subdir, public=self.cb['public'], family=self.cb['family'], friends=self.cb['friends'], update=self.chkUpdate.GetValue())
        
        thread2 = threading.Thread(target=self.longRunning, name='Thread-2', args=())
        thread2.start()
    ...
    def longRunning(self):
        ...
        while thread1.isAlive():
            ...
            if d['exitFlag']:
                ...
                try:
                    self.album.SetLabel('{}'.format(d['title']))
                    self.album_id.SetLabel('{}'.format(d['album_id']))
                    self.fname.SetLabel('{}'.format(d['fname']))
                    self.prog.SetLabel('{} of {}'.format(d['pcnt'],d['img_cnt']))
                    self.MD5.SetLabel('{}'.format(d['md5']))
                    self.SHA1.SetLabel('{}'.format(d['sha1']))
                    self.g_msg.SetLabel('{}'.format(d['g_msg']))
                    self.u_msg.SetLabel('{}'.format(d['u_msg']))


                    wx.CallAfter(self.g_albums.SetRange, int(d['mmax']))
                    wx.CallAfter(self.g_albums.SetValue, int(d['mcnt']))

                    # Set gauge for individual foldecnt
                    wx.CallAfter(self.g_photos.SetRange, int(d['img_cnt']))
                    wx.CallAfter(self.g_photos.SetValue, int(d['pcnt']))
                except:
